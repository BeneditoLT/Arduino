...
void setup () {  
  Serial.begin ( 9600 ) ; // Configura a porta seria para 9600 kbp/s
  analogReference(EXTERNAL);// Configura uma tensasão de referencia externa
  
}
void loop () {  
 
  valor = analogRead ( A0 );            // Carrega a variavel valor com o resultado medido em A0     
  valortr = valor * ( 1740 / 1024.0 );  // Converte este valor para volts 
